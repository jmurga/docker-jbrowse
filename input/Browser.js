...
initView: function() {
    ...
    return this._milestoneFunction('initView', function( deferred ) {
        ...
        // Custom div ********************************* 
        // Create a header over to the main menu
        var header = dojo.create( 'div', { id: 'idDiv_header', class: 'classDiv_header' }, topPane );
        
        var logo = dojo.create ( 'a', { class: 'classA_logo', innerHTML: ''
        ...
        var DataDescription = this.Data();
        var DataDescription_dialog = new InfoDialog(
            {
        ...
        var aboutDest = this.Dest();
        var aboutDestDialog = new InfoDialog(
            {
                title: aboutDest.title,
                content: aboutDest.description,
                className: 'about-dialog'
            });
...
/*        var aboutDownloadDialog = new InfoDialog(
        ...
        var resourcesDifferentiation = this.Differentiation();
        var resourcesDifferentiationDialog = new InfoDialog(
            {
...
*/
        ...
        */
        ...
        if( this.config.show_nav ) {
                                    ...
                                    new dijitMenuItem(
                                        {
                                            ...
                                            + 'document.getElementById("hideDialog").checked = localStorage.getItem("alerted");'
                                        })
                                    );
            ...
            this.addGlobalMenuItem( 'help', new dijitMenuSeparator() ); // Add a separator

            this._addDataDescription = new dijitMenuItem(
                                        {
                                            id: 'menubar_datadescription',
                                            label: 'Data description',
                                            iconClass: 'dijitIconEditProperty',
                                            onClick: function(){
                                                //var dataMap = 'https://infogr.am/298552e6-f2b5-49ea-92f3-f943c4b49893';
                                                //document.getElementById('dataMap').src = dataMap;
                                                DataDescription_dialog.show();
                                            }
                                        }
                                    );
            this.addGlobalMenuItem( 'help', this._addDataDescription );

            this._addDifferentiation = new dijitMenuItem(
                                        {
                                            id: 'menubar_resources_differentiation',
                                            label: 'Population differentiation',
                                            //title: 'Differentiation of...',
                                            iconClass: 'dijitIconUsers',
                                            onClick: dojo.hitch( resourcesDifferentiationDialog, 'show' )
                                        }
                                    );
            this.addGlobalMenuItem( 'help', this._addDifferentiation );

            this.addGlobalMenuItem( 'help', new dijitMenuSeparator() ); // Add a separator

            this._addTracksDescription = new dijitMenuItem(
                                        {
            ...
            this._addQnA = new dijitMenuItem(
                                        {
                                            id: 'menubar_qna',
                                            label: 'Dest Tutorial',
                                            iconClass: 'dijitIconFunction',
        //                                           onClick: function(){
        //                                               window.open('html/custom_QA.html','_blank','resizable=yes,toolbar=yes,scrollbars=yes,height=850,width=800');
        //                                           }
                                            onClick: dojo.hitch( tutorialDialog, 'show' )
                                        }
                                    );
            this.addGlobalMenuItem( 'help', this._addQnA );
            
            this.addGlobalMenuItem( 'help', new dijitMenuSeparator() ); // Add a separator

            this._aboutDestButton = new dijitMenuItem(
                                        {
                                            id: 'menubar_abouttheapp',
                                            label: 'About Dest',
                                            iconClass: 'dijitLeaf',
                                            onClick: dojo.hitch( aboutDestDialog, 'show' )
                                        }
                                    );
            this.addGlobalMenuItem( 'help', this._aboutDestButton );
...
// Functions *********************************
/**
 * Read an HTML file and get the content (body) in a single line
 */
readHTML: function(ruta) {
    ...
    define(["dojo/request/xhr"], function(xhr) {
        xhr.get(ruta, {                                                     // Read system filenode_modules/dojo/request/xhr.js
            ...
            function(data) {                                                // 'data' contains all the HTML
                contingut = data.replace(/\t/g,'');                         // Remove tabs
                contingut = contingut.replace(/\s\s/g,'');                  // Remove double spaces
                contingut = contingut.replace(/\n/g,'');                    // Remove newliners
                contingut = contingut.replace(/\.\.\//g,'');                // Remove path dependence ( ../../img/x.jpg >> img/x.jpg )
...
tutorial: function() {
    var tutorial = {};
    tutorial.title = tutorial.title || 'Dest Tutorial';
...
Differentiation: function() {
    var resourcesDifferentiation = {};
    resourcesDifferentiation.title = resourcesDifferentiation.title || 'Population differentiation';
    
    // Read the content from an HTML file (more confortable to edit)
    var html_file = "html/custom_PopulationDifferentiation.html";
    resourcesDifferentiation.description = this.readHTML(html_file);

    return resourcesDifferentiation;
},
// *********************************
